...
export default function Oscars() {
    ...
    const illustrationMaker = (item, movies, people) => {
        if (item.poster && movies[item.poster] && movies[item.poster].poster) {
            return <img src={getTMDBImage(movies[item.poster].poster)} />;
        } else if (item.profile && people[item.profile] && people[item.profile].profile) {
            return <img src={getTMDBImage(people[item.profile].profile)} />;
        } else {
            if (item.profile) {
